...
class BLIP_Retrieval(nn.Module):
    ...
    def forward(self, image, caption, alpha, idx):
        ...
        text_logit = F.normalize(text_output.last_hidden_state, dim=-1) #自己加的
...
##==================KD==============================
        ...
        with torch.no_grad():
            for i in range(len(encoder_input_ids_chunk)):  #遍历一个batch里的文字
                ...
                # 获取最后一层的Query向量
                Q_text = self.text_encoder.encoder.layer[11].attention.self.query(text_output.last_hidden_state)
