...
#    assert vit in ['base', 'large'], "vit parameter must be base or large"
    if vit=='small':
        vision_width = 768
        visual_encoder = VisionTransformer(img_size=image_size, patch_size=16, embed_dim=vision_width, depth=3, 
                                           num_heads=12, use_grad_checkpointing=use_grad_checkpointing, ckpt_layer=ckpt_layer,
                                           drop_path_rate=0 or drop_path_rate
                                          )   
    elif vit=='small6':
        vision_width = 768
        visual_encoder = VisionTransformer(img_size=image_size, patch_size=16, embed_dim=vision_width, depth=6, 
                                           num_heads=12, use_grad_checkpointing=use_grad_checkpointing, ckpt_layer=ckpt_layer,
                                           drop_path_rate=0 or drop_path_rate
                                          )   
...
#    print("state dict keys:", state_dict)
...
def load_checkpoint(model,url_or_filename):
    if is_url(url_or_filename):
        #cached_file = download_cached_file(url_or_filename, check_hash=False, progress=True)
        #checkpoint = torch.load('./annotation/model_base.pth', map_location='cpu') 
        checkpoint = torch.load('./output/retrieval_flickr/checkpoint_best.pth', map_location='cpu')
    elif os.path.isfile(url_or_filename):   
        #checkpoint = torch.load('./annotation/model_base.pth', map_location='cpu')      
        checkpoint = torch.load(url_or_filename, map_location='cpu') 
    else:
