...
class BLIP_Retrieval(nn.Module):
    ...
    def forward(self, image, caption, alpha, idx):
        ...
        image_embeds = self.visual_encoder(image)  #torch.Size([4, 577, 768])    len(image_q)=6 每个torch.Size([4, 12, 577, 64])  self.visual_encoder.blocks[0].attn.qkv
...
##==================KD==============================
        encoder_input_ids = text.input_ids.clone() # torch.Size([4, 35])
        encoder_input_ids[:,0] = self.tokenizer.enc_token_id #torch.Size([4])
        encoder_input_ids_chunk = torch.repeat_interleave(encoder_input_ids, encoder_input_ids.shape[0], 0).chunk(encoder_input_ids.shape[0])  #tuple len=4 每个torch.Size([4, 35])
        attention_mask_chunk = torch.repeat_interleave(text.attention_mask, encoder_input_ids.shape[0], 0).chunk(encoder_input_ids.shape[0]) #tuple len=4 每个torch.Size([4, 35])
        image_embeds_chunk = image_embeds.repeat(image_embeds.shape[0], 1, 1).chunk(encoder_input_ids.shape[0]) #tuple len=4 每个torch.Size([4, 577, 768])
        image_atts_chunk = image_atts.repeat(image_embeds.shape[0], 1).chunk(encoder_input_ids.shape[0]) #tuple len=4 每个torch.Size([4, 577)
  ...
  (encoder): BertEncoder(
    (layer): ModuleList(
      ...
      (11): BertLayer(
        ...
        # for i in range(6):
        #     # 获取第一层的Query向量
        #     Q = self.text_encoder.encoder.layer[i].attention.self.query(text_output.last_hidden_state)

        #     # 获取第一层的Key向量
        #     K = self.text_encoder.encoder.layer[i].attention.self.key(text_output.last_hidden_state)

        #     # 获取第一层的Value向量
        #     V = self.text_encoder.encoder.layer[i].attention.self.value(text_output.last_hidden_state)
        ...
        # for i in range(6):
        #     qkv=self.visual_encoder.blocks[i].attn.qkv  # Linear(in_features=768, out_features=2304, bias=True)
