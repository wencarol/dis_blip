...
def evaluation_two_tower(model, data_loader, device, config):
    ...
    text_logits = torch.cat(text_logits, dim=0)     # 自己加的
    text_logit_feat = F.normalize(text_embeds, dim=-1)    # 自己加的
    ...
    image_logit = F.normalize(image_feats,dim=-1)    # 自己加的 torch.Size([1014, 577, 768])
    image_logit_embed = F.normalize(image_embeds, dim=-1) # 自己加的
