...
@torch.no_grad()
def evaluation_two_tower(model, data_loader, device, config):
    # test
    model.eval() 
 ...
 * By Junnan Li
'''
from models.med import BertEncoder
...
def train(model, data_loader, optimizer, epoch, device, config,replacing_rate_scheduler,replacing_rate_scheduler_m):
    # train
    model.train()  
    ...
    for i,(image, caption, idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss.backward()
        optimizer.step()   
        replacing_rate_scheduler.step()
        replacing_rate_scheduler_m.step() 
...
def main(args, config):
    ...
    model = blip_retrieval(pretrained=config['pretrained'], image_size=config['image_size'], vit=config['vit'], 
                             vit_grad_ckpt=config['vit_grad_ckpt'], vit_ckpt_layer=config['vit_ckpt_layer'], 
                             queue_size=config['queue_size'], negative_all_rank=config['negative_all_rank'])
    summary(model, (16, 577, 768), device=args.device)
    scc_n_layer = model.text_encoder.encoder.scc_n_layer
    model.text_encoder.encoder.scc_layer = nn.ModuleList([deepcopy(model.text_encoder.encoder.layer[ix]) for ix in range(scc_n_layer)])
    model.text_encoder_m.encoder.scc_layer = nn.ModuleList([deepcopy(model.text_encoder_m.encoder.layer[ix]) for ix in range(scc_n_layer)])
    ...
    optimizer = torch.optim.AdamW(params=model.parameters(), lr=config['init_lr'], weight_decay=config['weight_decay'])
    replacing_rate_scheduler = ConstantReplacementScheduler(bert_encoder=model.module.text_encoder.encoder,
                                                            replacing_rate=0.5,
                                                            replacing_steps=1)
    replacing_rate_scheduler_m = ConstantReplacementScheduler(bert_encoder=model.module.text_encoder_m.encoder,
                                                            replacing_rate=0.5,
                                                            replacing_steps=1)
    best = 0
    best_epoch = 0
    ...
    for epoch in range(0, config['max_epoch']):    
        if not args.evaluate:        
            ...
            train_stats = train(model, train_loader, optimizer, epoch, device, config,replacing_rate_scheduler,replacing_rate_scheduler_m)
